    ...
    void saveDoubleArray(char *filename, TArray2D<double> *array)
{
	...
	double returnDoubleArray(int x, int y,  TArray2D<double> *array)
	{
...
int savePNG(std::string filename, TArray2D<SRGBA> * array) {
	...
	{
		anl::SRGBA val = array->get(x, y);
		image[4 * width * y + 4 * x + 0] = val[0] * 255;
...
/*int savePNG_bw(std::string filename, TArray2D<double> * array) {
	...
	for (unsigned x = 0; x < width; x++)
	{
		double val = array->get(x, y);
		val = std::max(0.0, std::min(1.0, val));
		image[3 * width * y + 3 * x + 0] = image[3 * width * y + 3 * x + 1] = image[3 * width * y + 3 * x + 2] = val * 255;
	}
